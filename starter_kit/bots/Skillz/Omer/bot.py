    ...
    for pirate in lstPirates:
        ...
        if pirate.mode == ATTACKING:
            ...
            if len(game.enemy_pirates()) > 0 and (distance(closestEnemyByTarget, pirate.target) <= 6 or distance(closestEnemyByGroup, group) <= 6):
                # if target is an island - save it!
            ...
            # if enemy is not close
                ...
                if pirate.target in game.my_islands():
                    ...
                    if len(running) > 0:
                        ...
                        # assign new target
                        pass
                # if enemy captured original target recapture it
                elif is_island(pirate.prevTarget) and not pirate.prevTarget in game.my_islands():
                    pirate.target = pirate.prevTarget

            # if killed enemy target
            if is_group(pirate.target) and pirate.target[0] in game.enemy_lost_pirates():
        ...
        if pirate.mode == ASSISTING:
            # if assisted pirate is lost - take his target
            if pirate.target in game.my_lost_pirates():
                pirate.target = pirate.target.prevTarget
                pirate.mode = ATTACKING
            # if pirate needs no assist anymore return to original target
            elif not (len(game.enemy_pirates()) > 0 and (distance(closestEnemyByTarget, pirate.target.prevTarget) <= 5 or distance(closestEnemyByGroup, group) <= 5)):
                pirate.target = pirate.prevTarget
                pirate.mode = ATTACKING
